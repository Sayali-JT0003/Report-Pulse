...
if file_uploaded is not None:
    ...
    def get_doctor_n_ward():
        st.session_state[severity_prediction+"_Doctor"] = "DR Suresh"
        st.session_state[severity_prediction+"_Ward"] = "101 General"
    ...
    if severity_prediction == 'Low':
        if st.button('Low Severity Button'):
            # Code to run when the Low Severity button is clicked
        
            st.write('Low Severity Button Clicked')
            get_doctor_n_ward()
            if st.session_state["Low_Doctor"]:
                st.write(st.session_state["Low_Doctor"],st.session_state["Low_Ward"])
    elif severity_prediction == 'Medium':

        if st.button('Medium Severity Button'):
            # Code to run when the Medium Severity button is clicked
            st.write('Medium Severity Button Clicked')
            get_doctor_n_ward()
            if st.session_state["Medium_Doctor"]:
                st.write(st.session_state["Medium_Doctor"],st.session_state["Medium_Ward"])
    elif severity_prediction == 'High':
        if st.button('High Severity Button'):
            # Code to run when the High Severity button is clicked
            st.write('High Severity Button Clicked')
            get_doctor_n_ward()
            if st.session_state["High_Doctor"]:
                st.write(st.session_state["High_Doctor"],st.session_state["High_Ward"])
